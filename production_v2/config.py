...
class V2Settings:
    ...
    ingest_auto_detect_gpus: bool = _env_bool("V2_INGEST_AUTO_DETECT_GPUS", True)
    ingest_dynamic_sharding_min_files: int = int(_env("V2_INGEST_DYNAMIC_SHARDING_MIN_FILES", "64"))
    ingest_embed_batch_min: int = int(_env("V2_INGEST_EMBED_BATCH_MIN", "16"))
    ingest_embed_batch_max: int = int(_env("V2_INGEST_EMBED_BATCH_MAX", "256"))
    ingest_bulk_chunk_min: int = int(_env("V2_INGEST_BULK_CHUNK_MIN", "200"))
    ingest_bulk_chunk_max: int = int(_env("V2_INGEST_BULK_CHUNK_MAX", "2000"))
    ingest_governor_enable: bool = _env_bool("V2_INGEST_GOVERNOR_ENABLE", True)
    ingest_bulk_retries: int = int(_env("V2_INGEST_BULK_RETRIES", "3"))
