...
try:
    from opensearchpy.helpers import bulk as _os_bulk, parallel_bulk as _os_parallel_bulk  # type: ignore
except Exception:
    _os_bulk = None
    _os_parallel_bulk = None
...
_PROD_ENSURED_INDEXES: set[str] = set()


def _ensure_prod_type_index(index_name: str) -> None:
    if index_name in _PROD_ENSURED_INDEXES:
        return
    client = get_opensearch_client()
    if client.indices.exists(index=index_name):
        _PROD_ENSURED_INDEXES.add(index_name)
        return
    dim = int(os.environ.get("AUSLEGALSEARCH_EMBED_DIM", "768"))
    total_fields_limit = int(os.environ.get("OPENSEARCH_TOTAL_FIELDS_LIMIT", "5000"))
    body = {
        "settings": {
            "index": {
                "knn": True,
                "number_of_shards": int(os.environ.get("OPENSEARCH_NUMBER_OF_SHARDS", "1")),
                "number_of_replicas": int(os.environ.get("OPENSEARCH_NUMBER_OF_REPLICAS", "0")),
        ...
        "mappings": {
            "properties": {
                ...
                "vector": {
                    ...
                    "dimension": dim,
                    "method": {
                        "name": os.environ.get("OPENSEARCH_KNN_METHOD", "hnsw"),
                        "space_type": os.environ.get("OPENSEARCH_KNN_SPACE", "cosinesimil"),
...
def _bulk_upsert_file_chunks_opensearch_production(
    ...
    if _os_bulk is None or _os_parallel_bulk is None:
        raise RuntimeError("opensearch-py helpers unavailable for production OpenSearch bulk ingestion")
    ...
    def _iter_actions():
        for i, chunk in enumerate(chunks):
            ...
            md = _store._sanitize_chunk_metadata_for_os((chunk or {}).get("chunk_metadata") or {})
            if _store._os_bool("OPENSEARCH_DROP_METADATA", False):
                md = None

            doc_key = _store._chunk_doc_key(source_path, abs_i, text_body)
...
# Override default OpenSearch writer with production type-index routing.
bulk_upsert_file_chunks_opensearch = _bulk_upsert_file_chunks_opensearch_production
